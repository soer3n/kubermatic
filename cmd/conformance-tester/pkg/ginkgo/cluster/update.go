...
func Update(rootCtx context.Context,
	...
	err := runtimeOpts.SeedClusterClient.Get(rootCtx, types.NamespacedName{Name: name}, cluster)
	Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to get cluster %s: %v", name, err))
	By(k8cginkgo.KKP("Updating Cluster"), func() {
		currentVersion := cluster.Spec.Version.Semver()
		nextRelease := fmt.Sprintf("%d.%d", currentVersion.Major(), currentVersion.Minor()+1)

		nextVersion := test.LatestKubernetesVersionForRelease(nextRelease, legacyOpts.KubermaticConfiguration)
		if nextVersion == nil {
			err = fmt.Errorf("cannot update cluster as there is no %s.x version configured", nextRelease)
		}
		Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to get cluster update version %s: %v", name, err))
		...
		err = runtimeOpts.SeedClusterClient.Patch(rootCtx, cluster, ctrlruntimeclient.MergeFrom(oldCluster))
		Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to patch cluster with new version %q: %w", nextVersion, err))
		time.Sleep(30 * time.Second) // wait a bit for the cluster-controller to react to the update before we check for reconciliation
	})
	...
	By(k8cginkgo.KKP("Wait for successful reconciliation"), func() {
		...
		Eventually(func() bool {
			...
			return false
		}).WithTimeout(15*time.Minute).WithPolling(15*time.Second).Should(BeTrue(), "cluster did not become healthy within the timeout")
	})
	...
	By(k8cginkgo.KKP("Wait for control plane"), func() {
		...
		Eventually(func() bool {
			...
			return err == nil
		}).WithTimeout(15 * time.Minute).WithPolling(15 * time.Second).Should(BeTrue())
	})
